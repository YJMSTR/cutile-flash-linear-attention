...
from fla_paddle.utils import IS_AMD, autotune_cache_kwargs, input_guard
...
def l2norm_fwd(
    ...
    if output_dtype is None:
        ...
        y = paddle.empty_like(x).cast(output_dtype)
    ...
    rstd = paddle.empty([T], dtype=paddle.float32)
    if D <= 512:
    ...
    return y.reshape(x_shape_og), rstd.reshape(x_shape_og[:-1])
...
def l2norm_bwd(
    ...
    return dx.reshape(y_shape_og)


class L2NormFunction(paddle.autograd.PyLayer):
    @staticmethod
    @input_guard
    def forward(
        ctx,
