...
from fla_paddle.ops.gated_delta_rule.wy_fast import recompute_w_u_fwd
from fla_paddle.ops.utils import prepare_chunk_indices
from fla_paddle.ops.utils.op import exp, exp2
from fla_paddle.utils import IS_TF32_SUPPORTED, autotune_cache_kwargs
...
def chunk_gated_delta_rule_fwd_intra(
    ...
    Args:
        ...
        chunk_indices (paddle.Tensor):
            Precomputed chunk indices. Default: `None`.

    Returns:
        w (paddle.Tensor): shape `[B, T, HV, K]`
        u (paddle.Tensor): shape `[B, T, HV, V]`
        A (paddle.Tensor): shape `[B, T, HV, BT]`, the solved (I+A)^{-1} matrix
    """
    ...
    A = paddle.zeros([B, T, HV, BT], dtype=k.dtype)
