...
from fla_paddle.utils import IS_NVIDIA_BLACKWELL, autotune_cache_kwargs, check_shared_mem
...
def recompute_w_u_fwd(
    ...
    w = paddle.empty([B, T, HV, K], dtype=k.dtype)
...
def prepare_wy_repr_bwd(
    ...
    dk = paddle.empty([B, T, HV, K], dtype=k.dtype)
    dv = paddle.empty_like(v)
    dg = paddle.empty_like(g) if g is not None else None
    db = paddle.empty_like(beta)
