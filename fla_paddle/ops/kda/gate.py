...
from fla_paddle.ops.utils.op import exp
from fla_paddle.ops.utils.softplus import softplus
from fla_paddle.utils import IS_AMD, autocast_custom_bwd, autocast_custom_fwd, autotune_cache_kwargs, check_shared_mem, input_guard
from fla_paddle.triton_utils import enable_compat_on_triton_kernel
...
def naive_kda_gate(
    ...
    """
    Paddle reference implementation for KDA gate computation.
    ...
    g = (-A_log.reshape([H, 1]).cast(paddle.float32).exp() * F.softplus(g.cast(paddle.float32))).cast(output_dtype)
    return g
...
def naive_kda_lowerbound_gate(
    ...
    if dt_bias is not None:
        g = g + dt_bias.reshape([H, -1])
    g = lower_bound * F.sigmoid(A_log.reshape([H, 1]).exp() * g)
    return g.cast(output_dtype)
...
def kda_gate_bwd(
    ...
    # PyTorch's .sum() on float16 internally promotes to float32 for accumulation,
    # but Paddle's .sum() accumulates directly in the input dtype. Computing dbias
    # in float32 avoids precision loss when summing over B*T elements.
    dbias = dg.reshape([-1, H * K]).sum(axis=0).cast(dt_bias.dtype) if dt_bias is not None else None
...
class KDAGateFunction(paddle.autograd.PyLayer):
    @staticmethod
    @input_guard
    @autocast_custom_fwd
    def forward(
        ...
        _forward_args = [g, A_log, dt_bias, lower_bound, output_dtype]
    ...
    def backward(ctx, dyg: paddle.Tensor):
        ...
        all_grads = [dg, dA, dbias, None, None]
        return tuple(
