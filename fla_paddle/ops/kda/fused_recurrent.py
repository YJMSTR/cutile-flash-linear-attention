...
from fla_paddle.utils import input_guard
...
def fused_recurrent_kda_fwd(
    ...
    ssm_state_indices: paddle.Tensor | None = None,
    num_accepted_tokens: paddle.Tensor | None = None,
...
def fused_recurrent_kda(
    ...
    **kwargs,
    ...
    Examples::
        ...
        >>> from fla_paddle.ops.kda import fused_recurrent_kda
        # inputs with equal lengths
        >>> B, T, H, HV, K, V = 4, 2048, 4, 8, 512, 512
        >>> q = paddle.randn([B, T, H, K])
        >>> k = F.normalize(paddle.randn([B, T, H, K]), p=2, axis=-1)
        >>> v = paddle.randn([B, T, HV, V])
        >>> g = F.log_sigmoid(paddle.rand([B, T, HV, K]))
        >>> beta = paddle.rand([B, T, HV]).sigmoid()
        >>> h0 = paddle.randn([B, HV, K, V])
        ...
        >>> cu_seqlens = paddle.to_tensor([0, 2048, 4096, 6144, 8192], dtype=paddle.int64)
