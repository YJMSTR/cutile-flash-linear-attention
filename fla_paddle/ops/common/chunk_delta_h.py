...
from fla_paddle.ops.utils import prepare_chunk_indices, prepare_chunk_offsets
from fla_paddle.ops.utils.op import exp, exp2
from fla_paddle.utils import IS_NVIDIA_HOPPER, USE_CUDA_GRAPH, autotune_cache_kwargs, check_shared_mem
...
def chunk_gated_delta_rule_fwd_h(
    ...
    if transpose_state_layout:
        h = paddle.empty(shape=[B, NT, HV, V, K], dtype=k.dtype)
        final_state = paddle.zeros(shape=[N, HV, V, K], dtype=paddle.float32) if output_final_state else None
    else:
        h = paddle.empty(shape=[B, NT, HV, K, V], dtype=k.dtype)
        final_state = paddle.zeros(shape=[N, HV, K, V], dtype=paddle.float32) if output_final_state else None
