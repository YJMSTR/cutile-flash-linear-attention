...
We are exploring the use of [cuTile](https://github.com/NVIDIA/cutile-python), an emerging DSL from NVIDIA, to implement high-performance linear attention kernels. cuTile provides a tile-based programming model in Python that is close to the metal while remaining highly productive.
...
To verify the correctness of the cuTile implementations:
...
To compare the performance of cuTile implementations with Naive (PyTorch) and Triton versions:
